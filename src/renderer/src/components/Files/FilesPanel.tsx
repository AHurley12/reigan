...
export function FilesPanel() {
  ...
  return (
    <div className="flex h-full">
      {/* List column */}
      <div className="flex flex-col shrink-0" style={{ width: selected ? 420 : '100%', borderRight: selected ? '1px solid var(--border)' : 'none' }}>
        ...
        {indexStatus && (
          <div className="px-6 py-1.5 text-[10px] font-mono" style={{ color: 'var(--text-muted)' }}>
        ...
        {!searchMode && (
          <div className="flex items-center gap-1 px-4 pt-2.5 text-[11px] overflow-x-auto" style={{ color: 'var(--text-muted)' }}>
            <button
              onClick={handleGoUp}
              disabled={!currentDir || !homeDir || currentDir.toLowerCase() === homeDir.toLowerCase()}
              className="w-5 h-5 rounded flex items-center justify-center hover:bg-white/5 disabled:opacity-30 shrink-0"
        ...
        {/* Filters */}
        <div className="flex flex-col gap-2 px-4 pt-2.5 pb-2.5" style={{ borderBottom: '1px solid var(--border)' }}>
            ...
            {FILE_TYPE_CATEGORIES.map((c) => {
              ...
              return (
                <button
                  ...
                  style={{
                    background: active ? 'rgba(35, 161, 140, 0.14)' : 'transparent',
          ...
          <div className="flex items-center gap-2">
            <select
              ...
              className="text-[11px] rounded-md px-2 py-1 bg-transparent focus:outline-none"
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {searchMode && (
                    <p className="text-[10px] truncate mt-0.5" style={{ color: 'var(--text-muted)' }}>{entry.dir}</p>
                  )}
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  {!entry.isDir && (
                    <span className="text-[10px] font-mono" style={{ color: 'var(--text-muted)' }}>{formatBytes(entry.size)}</span>
                  )}
                  <span className="text-[10px] font-mono w-14 text-right" style={{ color: 'var(--text-muted)' }}>{formatDate(entry.mtime)}</span>
                </div>
              </div>
            )
          })}
      ...
      {selected && (
        <div className="flex-1 flex flex-col overflow-hidden">
          <div className="flex items-center gap-2 px-6 py-4 shrink-0" style={{ borderBottom: '1px solid var(--border)' }}>
            {(() => {
            ...
            <button
              ...
              className="text-[10px] px-2 py-1 rounded-md transition-colors hover:bg-white/5 shrink-0 flex items-center gap-1"
              style={{ border: '1px solid var(--border)', color: 'var(--text-muted)' }}
            >
          ...
          <div className="px-6 py-3 flex flex-wrap gap-x-4 gap-y-1 text-[11px]" style={{ borderBottom: '1px solid var(--border)', color: 'var(--text-muted)' }}>
